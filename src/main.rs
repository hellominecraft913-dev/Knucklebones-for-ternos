...
// Stack string writer for embedded/no_std environment
struct BufWriter<'a> {
    buf: &'a mut [u8],
    pos: usize,
}
...
impl KnucklebonesApp {
    ...
    fn ai_choose_column(&mut self, val: u8) -> usize {
        ...
        for c in 0..3 {
            if self.ai_counts[c] < 3 {
                ...
                let temp_p: Vec<u8> = self.player_grid[c][..self.player_counts[c]]
                    .iter()
                    .cloned()
                    .filter(|&v| v != val)
                    .collect();
                let destroyed = self.player_counts[c] - temp_p.len();
                let new_p = Self::col_score(&temp_p, temp_p.len());
                let loss = (old_p - new_p) as i32;
        ...
        D: DrawTarget<Color = BinaryColor>,
    {
        ...
        // Center Action & Status Display (Y: 280..420)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut app = KnucklebonesApp::new();

    // Native XTEINK X3 480x800 display resolution
    let mut display: SimulatorDisplay<BinaryColor> = SimulatorDisplay::new(Size::new(480, 800));
    ...
    let mut window = Window::new("TernOS - Knucklebones", &output_settings);
        ...
        for event in window.events() {
            match event {
                ...
                SimulatorEvent::KeyDown { keycode, .. } => {
                    match format!("{:?}", keycode).as_str() {
                        "Left" => {
                            app.handle_button(Button::Left);
                            needs_redraw = true;
                        }
                        "Right" => {
                            app.handle_button(Button::Right);
                            needs_redraw = true;
                        }
                        "Return" | "Space" => {
                            app.handle_button(Button::Select);
                            needs_redraw = true;
                        }
                        "Escape" | "Backspace" => {
                            app.handle_button(Button::Back);
                            needs_redraw = true;
                        }
                        _ => {}
                    }
